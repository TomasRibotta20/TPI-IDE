        ...
        public async Task<AlumnoCursoDto> InscribirAlumnoAsync(int idAlumno, int idCurso, CondicionAlumnoDto condicion = CondicionAlumnoDto.Regular)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                try
                {
                    ...
                    // Si no se pudo antes, intentamos deserializar el error
                ...
                {
                    // Si no podemos deserializar el error pero tenemos contenido, lo usamoos
                ...
                // Si no pudimos extraer un mensaje específico, usamos un mensaje genérico
        ...
        public async Task ActualizarCondicionYNotaAsync(int idInscripcion, CondicionAlumnoDto condicion, int? nota = null)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                switch (response.StatusCode)
                {
                    ...
                    case System.Net.HttpStatusCode.BadRequest:
                        // Intentamos extraer mensaje específico del BadRequest
                        try
                        {
                            var errorObj = JsonSerializer.Deserialize<JsonElement>(errorContent);
                            if (errorObj.TryGetProperty("detail", out var detail))
                        ...
                        {
                            // Si no es JSON válido, usamos el contenido como está
                            if (!string.IsNullOrEmpty(errorContent))
                            {
                                throw new Exception(errorContent);
                            }
