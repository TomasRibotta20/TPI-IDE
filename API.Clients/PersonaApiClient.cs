...
namespace API.Clients
{
    public class PersonaApiClient : BaseApiClient
    {
        ...
        public async Task<PersonaDto> CreateAsync(PersonaDto persona)
        {
            ...
            // Leemos el objeto creado desde la respuesta
            var personaCreada = await response.Content.ReadFromJsonAsync<PersonaDto>();
            if (personaCreada == null)
            {
                throw new Exception("No se pudo deserializar la persona creada");
            }
            
            return personaCreada;
        }

        public async Task<PersonaDto> UpdateAsync(int id, PersonaDto persona)
        {
            using var client = await CreateHttpClientAsync();
