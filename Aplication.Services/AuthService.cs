    ...
    public class AuthService
    {
        ...
        public async Task<RegisterResponseDto> RegisterAsync(RegisterRequestDto request)
        {
            try
            {
                ...
                // 3. Auto-generar legajo si no se proporcionó
                int legajoFinal;
                if (request.Legajo.HasValue && request.Legajo.Value > 0)
                {
                    // Validar que el legajo proporcionado no exista
                    if (personas.Any(p => p.Legajo == request.Legajo.Value))
                    {
                        return new RegisterResponseDto
                        {
                            Success = false,
                    ...
                    legajoFinal = request.Legajo.Value;
                }
                else
                {
                    // Auto-generar legajo: tomar el máximo + 1
                    legajoFinal = personas.Any() ? personas.Max(p => p.Legajo) + 1 : 1000;
                }
                ...
                {
                    Success = true,
                    Message = $"Usuario {request.TipoPersona} registrado exitosamente con legajo {legajoFinal}",
